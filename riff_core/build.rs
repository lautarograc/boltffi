...
fn main() {
    ...
    let header_path = out_dir.join("riff_core.h");
...
fn generate_export_declaration(export: &FfiExport) -> String {
    ...
    match &export.return_kind {
        FfiReturnKind::Vec(inner_ty) => {
            ...
            format!(
                "uintptr_t riff_{}_len({});\nstruct FfiStatus riff_{}_copy_into({});\n",
                export.name,
                len_params,
                export.name,
                copy_params.join(", ")
        ...
        FfiReturnKind::AsyncPoll(result_type) => {
            ...
            out.push_str(&format!(
                "RustFutureHandle riff_{}({});\n",
                export.name, entry_params
            ));
            out.push_str(&format!(
                "void riff_{}_poll(RustFutureHandle handle, uint64_t callback_data, RustFutureContinuationCallback callback);\n",
                export.name
            ));
            out.push_str(&format!(
                "{} riff_{}_complete(RustFutureHandle handle, struct FfiStatus* out_status);\n",
                result_type, export.name
            ));
            out.push_str(&format!(
                "void riff_{}_cancel(RustFutureHandle handle);\n",
                export.name
            ));
            out.push_str(&format!(
                "void riff_{}_free(RustFutureHandle handle);\n",
                export.name
            ));
            out
        }
...
fn generate_stream_declarations(stream: &FfiStreamExport) -> String {
    let base_name = format!("riff_{}_{}", stream.class_name, stream.method_name);
    ...
    if let Some(pos) = header.rfind("#endif") {
        ...
        let generic_defs: String = generic_types
            ...
            .map(|(wrapper, inner, c_type)| {
                if wrapper == "FfiBuf" {
                    format!(
                        "typedef struct FfiBuf_{} {{\n  {} *ptr;\n  uintptr_t len;\n  uintptr_t cap;\n}} FfiBuf_{};\n\nvoid riff_free_buf_{}(struct FfiBuf_{} buf);\n\n",
                        inner, c_type, inner, inner, inner
                    )
                } else {
                    format!(
                        "typedef struct FfiOption_{} {{\n  bool isSome;\n  {} value;\n}} FfiOption_{};\n\nbool riff_option_{}_is_some(struct FfiOption_{} opt);\n\n",
                        inner, c_type, inner, inner, inner
                    )
                }
            })
...
fn generate_trait_typedef(t: &FfiTrait) -> String {
    ...
    format!(
        "typedef struct {} {{\n{}\n}} {};\n\n\
         typedef struct {} {{\n  const struct {} *vtable;\n  uint64_t handle;\n}} {};\n\n\
         void riff_register_{}_vtable(const struct {} *vtable);\n\
         struct {} *riff_create_{}(uint64_t handle);\n\n",
        vtable_name,
        vtable_fields.join("\n"),
        vtable_name,
        foreign_name,
